...
export class GameController {
  ...
  alternatePlayer() {
    this.currentPlayer =
      this.currentPlayer === this.realPlayer ? this.botPlayer : this.realPlayer;
  ...
  botPlace() {
    ...
    ships.forEach(([type]) => {
      ...
      while (!placed) {
        ...
        placed = this.botPlayer.gameBoard.place(type, axis, x, y, true);
      }
    });
  }
  checkWin(opponent) {
    if (opponent.gameBoard.allSunk()) {
      alert(
        `${this.currentPlayer === this.realPlayer ? "Player" : "Bot"} wins!`,
      );
