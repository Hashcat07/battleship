...
export class GameBoard {
  ...
  place(type, axis, xAxis, yAxis, isBot = false) {
    ...
    if (!this.shipUsed.includes(type) && shipLength !== null) {
      ...
      for (let i = 0; i < shipLength; i++) {
        ...
        if (this.board[checkX][checkY] instanceof Ship && !isBot) {
          alert(`Overlap detected at [${checkX},${checkY}] for ${type}`);
          return false;
        }
      }
  ...
  receiveAttack(xAxis, yAxis) {
    if (xAxis > 9 || yAxis > 9 || xAxis < 0 || yAxis < 0) return "Out of Board";
